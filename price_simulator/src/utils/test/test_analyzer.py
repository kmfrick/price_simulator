...
def test_prepare_profit_calculation():
    env = DiscreteSynchronEnvironment(
        n_periods=1,
        agents=[Qlearning(), Qlearning(), Qlearning(), Qlearning()],
        demand=LogitDemand(),
    )
    env.play_game()
...
def test_get_collusion_for():
    assert (
        analyzer.get_collusion_for(np.array([2]), np.array([1]), np.array([4]))
        == np.array([1 / 3])
    ).all()
    assert (
        analyzer.get_collusion_for(np.array([2, 2]), np.array([1, 1]), np.array([4, 4]))
        == np.array([1 / 3, 1 / 3])
    ).all()
    assert (
        analyzer.get_collusion_for(np.array([1, 4]), np.array([1, 1]), np.array([4, 4]))
        == np.array([0, 1])
    ).all()
...
def test_analyze():
    ...
    nash_profits, monopoly_profits = analyzer.prepare_profit_calculation(env)
    collusion_profits = analyzer.get_collusion_for(
        average_profits, nash_profits, monopoly_profits
    )
    assert (collusion_profits == np.array([1.0, -1.0, 2.0, -0.5])).all()

    env = DiscreteSynchronEnvironment(
        agents=[Qlearning(marginal_cost=1.0), Qlearning(marginal_cost=0.0)],
        demand=ConstantDemand(),
    )
