...
def test_profit():
    ...
    assert EquilibriumCalculator(
        demand=LogitDemand(price_sensitivity=0.5, outside_quality=1.0)
    ).profit(
        4.1, np.array([10.0, 10.0]), np.array([2.0, 1.0]), np.array([4.0, 1.0]), 0
    ) < EquilibriumCalculator(
        demand=LogitDemand(price_sensitivity=0.5, outside_quality=1.0)
    ).profit(4.0, np.array([10.0, 10.0]), np.array([2.0, 1.0]), np.array([4.0, 1.0]), 0)
    assert (
...
def test_vector_reaction():
    assert (
        np.round(
            EquilibriumCalculator(
                demand=LogitDemand(price_sensitivity=0.8, outside_quality=1.0)
            ).get_nash_equilibrium([1.2, 1.0, 0.8], [1.0, 0.9, 0.8])[0],
            5,
        )
        == 1.88108  # noqa W503
    )
    assert (
        np.round(
            EquilibriumCalculator(
                demand=LogitDemand(price_sensitivity=0.8, outside_quality=1.0)
            ).get_nash_equilibrium([1.0, 1.0], [1000.0, 10000.0])[0],
            5,
        )
        > 0.0  # noqa W503
    )
    ...
    def profit(price, cost=1.0, quality=1.0):
        quantity = LogitDemand(
            price_sensitivity=0.005, outside_quality=1.0
        ).get_quantities((price,), (quality,))[0]
    ...
    assert np.round(
        ...
        minimize(
            profit, np.array([1]), method="nelder-mead", options={"xatol": 1e-8}
        ).x,
        4,
    )
...
def test_equilibrium_calculation():
    ...
    assert (
        np.around(
            EquilibriumCalculator(
                demand=LogitDemand(price_sensitivity=mu, outside_quality=a0)
    ...
    demand = np.around(
        EquilibriumCalculator(
            demand=LogitDemand(price_sensitivity=10000.0, outside_quality=1.0)
        ).get_nash_equilibrium(mcs, mcs),
        3,
    )
