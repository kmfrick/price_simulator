...
from price_simulator.src.algorithm.environment import (
    DiscreteSynchronEnvironment,
    ReformulationEnvironment,
)
from price_simulator.src.algorithm.policies import (
...
def test_environment_prisoners():
    test_1 = DiscreteSynchronEnvironment(
        ...
        agents=[
            Qlearning(
                discount=0.95, learning_rate=0.3, decision=EpsilonGreedy(eps=0.1)
            ),
            Qlearning(
                discount=0.95, learning_rate=0.3, decision=EpsilonGreedy(eps=0.1)
            ),
    ...
    test_2 = DiscreteSynchronEnvironment(
        ...
        agents=[
            Qlearning(
                discount=0.95, learning_rate=0.5, decision=DecreasingEpsilonGreedy()
            ),
            AlwaysDefectAgent(),
        ],
    )
    ...
    test_1 = DiscreteSynchronEnvironment(
        ...
        agents=[
            ...
            Qlearning(
                ...
                marginal_cost=4.0,
                quality=5.0,
                decision=EpsilonGreedy(eps=0.1),
            ),
...
def test_init():
    env = DiscreteSynchronEnvironment(
        ...
        agents=[
            Qlearning(quality=1.0, marginal_cost=0.0),
            Qlearning(quality=1.0, marginal_cost=0.0),
        ],
    )
    ...
    env = DiscreteSynchronEnvironment(
        ...
        agents=[
            Qlearning(quality=10.0, marginal_cost=5.0),
            Qlearning(quality=10.0, marginal_cost=1.0),
        ],
    )
    assert max(env.monopoly_prices) > min(env.nash_prices)
    assert sum(np.greater(env.nash_prices, env.monopoly_prices)) == 0
...
def test_reformulation():
    env = ReformulationEnvironment(
        demand=ConstantDemand(default_quantity=0.0),
        agents=[DiffDQN(), DiffDQN()],
        n_periods=5,
    )
    ...
    env = ReformulationEnvironment(
        demand=ConstantDemand(default_quantity=0.0),
        agents=[DiffDQN(), DiffDQN(), DiffDQN()],
        n_periods=5,
    )
    assert env.play_game() == 4
